...
module Terminating.LambdaCBName where

open import LambdaCBName hiding (_∎)
...
csize RET = 1
csize (APP c c') = suc (csize c + csize c')
...
mutual
  ...
  exec' _ (LOOKUP n c) (s , e) = do c' , e' <- getThunk' <$> getVar n e
                                    later (∞exec' c' (CLO c e  ∷ s , e'))
  exec' (suc j) RET  (VAL u ∷ CLO c e' ∷ s , _) = exec' j c (VAL u ∷ s , e')
  exec' _ (APP c' c) (VAL (Clo' c'' e') ∷ s , e) = later (∞exec' c'' (CLO c e ∷ s , thunk' c' e ∷ e'))
...
mutual
  ...
  execBisim (LOOKUP x c) (s , e) (suc i) _ (s≤s le) =
    bind-cong-r (getThunk' <$> getVar x e) ( λ { (c , e) → ~ilater (∞execBisim c _ _) })
  ...
  execBisim (APP c' c) ([] , e) (suc i) _ (s≤s le) = ~irefl
  execBisim (APP c' c) (VAL (Num' x) ∷ s , e) (suc i) _ (s≤s le) = ~irefl
  execBisim (APP c' c) (VAL (Clo' x x₁) ∷ s , e) (suc i) _ (s≤s le) = ~ilater (∞execBisim x _ _)
  execBisim (APP c' c) (CLO x x₁ ∷ s , e) (suc i) _ (s≤s le) =  ~irefl
